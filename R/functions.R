...
#' @param m is the dimension of the matrix.
#' @param rho is the correlation coefficient.
...
#' @param z_vector column vectorized data matrix with q rows m columns.
...
#' ThreeWayTest::MGAS(z_vector=z_vector, est_genetic_cor = genotype_covariance, est_pheno_cor =phenotype_covariance)
...
#' 
#' @param z_vector column vectorized data matrix with q rows m columns.
#' @param cov_mat is the estimated covariance matrix of z_vector.
...
#' @param z_vector column vectorized data matrix with q rows m columns.
#' @param cov_mat is the estimated covariance matrix of z_vector.
#' @param eta is the truncated value of eta.
#' @returns A numeric value represent calculated statistic T_eta.
#' @importFrom MASS ginv
#' @export
#' @examples
#' z_vector<-MASS::mvrnorm(1,mu=rep(0,9),Sigma = diag(nrow = 9, ncol = 9))
#' ThreeWayTest::T_eta(z_vector=z_vector, cov_mat=diag(nrow = 9, ncol = 9), eta=0.5)
...
#' @param cov_mat is the covariance matrix for generating random samples.
#' @param cutoff_value is the truncated value, should be in 0 to 1 and must contain 1.
...
#' @export
#' @examples 
#' ThreeWayTest::generate_null_distribution_T3(m=6,n=1000,cov_mat=diag(nrow = 6, ncol = 6), cutoff_value=c(0.2,0.4,0.6,0.8,1))
...
#' Please see Setp 2 in our manuscript for more information.
#' @param null_distribution null distribution matrix generated by generate_null_distribution_T3
#' @returns list of coefficient of alpha, beta and d.
#' @export
#' @examples
#' null_distribution<-ThreeWayTest::generate_null_distribution_T3(m=6,n=1000,cov_mat=diag(nrow = 6, ncol = 6), cutoff_value=c(0.2,0.4,0.6,0.8,1))
...
#' This function estimate the coeffcient of T3 with null distribution generated by  generate_null_distribution_T3.
#' Please see Setp 2 in our manuscript for more information.
#' @param null_distribution_matrix null distribution matrix generated by generate_null_distribution_T3
...
#' null_distribution<-ThreeWayTest::generate_null_distribution_T3(m=10,n=1000,cov_mat=diag(nrow = 10,ncol= 10), cutoff_value=c(0.2,0.4,0.6,0.8,1))
#' coefficient_matrix<-ThreeWayTest::approximate_distribution_coefficient_estimate_T3(null_distribution)
#' ThreeWayTest::T_3(z_vector=z_vector, cov_mat=diag(nrow = 10, ncol = 10), cutoff_value=c(0.2,0.4,0.6,0.8,1), coefficient_matrix=coefficient_matrix)
...
#' @param est_genetic_cor stands for the estimated correlation matrix of genetic variants
#' @param est_pheno_cor stands for the estimated correlation matrix of phenotypes
#' @param cutoff_value is the set Omega in our paper 
#' @param coefficient_matrix is calculated based on function approximate_distribution_coefficient_estimate_T3
#' @returns p_value_final is the p_value of TWT.
#' @returns p_1 is the p_value of T_1.
#' @returns p_2 is the p_value of T_2.
#' @returns p_3 is the p_value of T_3.
#' @export
#' @examples 
#' z_mat<-MASS::mvrnorm(5,mu=rep(0,5), Sigma = diag(nrow = 5, ncol = 5))
#' null_distribution<-ThreeWayTest::generate_null_distribution_T3(m=25,n=1000,cov_mat=diag(nrow = 25,ncol= 25), cutoff_value=c(0.2,0.4,0.6,0.8,1))
#' coefficient_matrix<-ThreeWayTest::approximate_distribution_coefficient_estimate_T3(null_distribution)
#' ThreeWayTest::TWT(z_mat=z_mat, est_genetic_cor=diag(nrow = 5, ncol = 5), est_pheno_cor=diag(nrow = 5, ncol = 5), cutoff_value=c(0.2,0.4,0.6,0.8,1), coefficient_matrix=coefficient_matrix)
...
TWT<-function(z_mat, 
  ...
  for (i in 1:number_of_snp) {
    pvalue_genetic[i]<-chisq_test(z_mat[,i],est_pheno_cor)
  }
  ...
  for (i in 1:number_of_pheno) {
    pvalue_pheno[i]<-chisq_test(z_mat[i,],est_genetic_cor)
  }
